...
/**
 * Looks for a clean<Name> or clean_name custom cleaning method and calls it,
 * expecting synchronous or asynchronous behaviour, depending on its arity.
 * @param {string} name a field name.
 */
BaseForm.prototype._cleanFieldCustom = function(name) {
  // Check for clean<Name>([cb]) or clean_<name>([cb])
  var customClean =
    (this['clean' + name.charAt(0).toUpperCase() + name.substr(1)] ||
     this['clean_' + name])

  if (!is.Function(customClean)) { return }

  var returnValue
  // Check arity to see if we have clean<Name>() or clean<Name>(cb)
  if (customClean.length == 1) {
    ...
    var callback = function(err, field, error) {
      ...
      if (err) {
        console.error('Error performing async validation of ' + name + ':', err)
      }
    ...
    returnValue = customClean.call(this, callback)
    ...
    returnValue = customClean.call(this)
    if (typeof returnValue != 'undefined') {
      this.cleanedData[name] = returnValue
    }
  }
}
...
BaseForm.prototype._handleFieldValidation = function(validation, e) {
  var data = this._updateFieldInputData(e)
  var fieldName =
    this.removePrefix(e.target.getAttribute('data-newforms-field') || e.target.name)
  ...
  if (validation.event == 'onBlur') {
    ...
    // XXX Does this check work for MultiValueFields?
    validate = (field.required && field.isEmptyValue(data))
  }

  // Only trigger validation if the data has changed since validation was last
  // performed - this prevents displayed validation errors being cleared
  // unnecessarily.
  if (!validate) {
    var lastValidatedData = object.get(this._lastValidatedData, fieldName, null)
    validate = field._hasChanged(lastValidatedData, data)
...
BaseForm.prototype._immediateFieldValidation = function(field) {
  ...
  this._lastValidatedData[field] = this.data[this.addPrefix(field)]
