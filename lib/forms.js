...
BaseForm.prototype.partialClean = function(fields) {
  ...
  // fields it uses, or one of the fields it uses was just cleaned.
...
BaseForm.prototype._cleanField = function(name) {
  ...
  try {
    ...
    if (is.Function(customClean)) {
      var cleanedValue = this._runCustomClean(name, customClean)
      if (typeof cleanedValue != 'undefined') {
        this.cleanedData[name] = cleanedValue
      }
    }
  }
  catch (e) {
...
BaseForm.prototype._runCustomClean = function(fieldName, customClean) {
  ...
  var callback = function(err, field, error) {
    ...
    if (err) {
      console.error('Error performing async validation of ' + fieldName + ':', err)
    }
...
BaseForm.prototype._cleanForm = function() {
  try {
    if (this.clean !== noop) {
      var cleanedData = this._runCustomClean(NON_FIELD_ERRORS, this.clean)
