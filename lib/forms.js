...
/**
 * Sets the form's entire input data, optionally triggering validation or using
 * the new data as initial data, effectively resetting the form.
 * @param {Object.<string,*>} data new input data for the form.
 * @param {Object.<string,boolean>} kwargs set options.
 * @retun {boolean} if set options indicate the new data should be validated,
 *    true if the new data is valid.
 */
BaseForm.prototype.setData = function(data, kwargs) {
  kwargs = object.extend({prefixed: false, validate: true, asInitial: false}, kwargs)

  this._changedData = null
  this.data = (kwargs.prefixed ? data : this._prefixData(data))

  if (kwargs.asInitial) {
    this.initial = (kwargs.prefixed ? this._deprefixData(data) : data)
  ...
  if (!kwargs.asInitial && kwargs.validate) {
    return isValid
  }
}
