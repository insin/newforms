...
BoundField.prototype.render = function(kwargs) {
  if (this.field.showHiddenInitial) {
    return env.render('div', null, this.asWidget(kwargs),
...
BoundField.prototype.labelTag = function(kwargs) {
  ...
  if (id) {
    ...
    contents = env.render('label', attrs, contents)
  }
  return contents
}
 ...
 * @return a list of components representing rows.
 */
  ...
  for (i = 0, l = visibleBoundFields.length; i < l; i++) {
    ...
    if (helpText) {
      helpText = ((is.Object(helpText) && object.hasOwn(helpText, '__html'))
                  ? env.render('span', {className: 'helpText', dangerouslySetInnerHTML: helpText})
                  : env.render('span', {className: 'helpText'}, helpText))
    }
...
BaseForm.prototype.asTable = (function() {
  function normalRow(key, cssClasses, label, field, helpText, errors, extraContent) {
    ...
    contents.push(field)
    if (helpText) {
      contents.push(env.render('br', null))
    ...
    return env.render('tr', rowAttrs
    , env.render('th', null, label)
    , env.render('td', null, contents)
    )
  }
  ...
  function errorRow(key, errors, extraContent, cssClasses) {
    ...
    if (cssClasses) { rowAttrs.className = cssClasses }
    return env.render('tr', rowAttrs
    , env.render('td', {colSpan: 2}, contents)
    )
  }
...
function _normalRow(el, key, cssClasses, label, field, helpText, errors, extraContent) {
  var rowAttrs = {key: key}
  if (cssClasses) { rowAttrs.className = cssClasses }
  var contents = []
  if (errors) { contents.push(errors) }
  ...
  if (extraContent) { contents.push.apply(contents, extraContent) }
  return env.render(el, rowAttrs, contents)
}

function _errorRow(el, key, errors, extraContent, cssClasses) {
  var rowAttrs = {key: key}
  if (cssClasses) { rowAttrs.className = cssClasses }
  var contents = []
  if (errors) { contents.push(errors) }
