...
var BaseForm = Concur.extend({
  constructor: function BaseForm(kwargs) {
    ...
    // Lookup for names of ields pending validation which clean() depends on
    this._runCleanAfter = {}
 ...
 * @param {Function(err, isValid)=} cb callback for asynchonous validation
 * @return {boolean|undefined} true if the form only has synchronous validation
 *   and is valid,
 * @throws if the form has asynchronous validation and a callback is not provided
 */
...
BaseForm.prototype._validateAsync = function(cb) {
  if (!is.Function(cb)) {
    throw new Error(
      'You must provide a callback to validate() when a form has custom ' +
      'cleaning methods which take a callback for asynchronous validation.'
    )
  }
  if (this.isInitialRender) {
    this.isInitialRender = false
...
BaseForm.prototype.boundFields = function(test) {
  ...
  for (var i = 0, l = fieldNames.length; i < l ; i++) {
    ...
    if (!test || test(this.fields[fieldName])) {
...
/**
 * @return {boolean} true if the form has any cleaning methods which take a
 *   callback argument.
 */
...
/**
 * Sets up pending validation state prior to cleaning fields Configures cross-field cleaning to run after its dependent fields have been
 ...
 * @param {string} fieldName
 * @param {errpr=} err an error caught while cleaning the field.
 */
