...
var BaseForm = Concur.extend({
  constructor: function BaseForm(kwargs) {
    ...
    /** Fields pending validation, after which clean() should be run. */
    this._runCleanAfter = {}
...
/**
 * Determines whether or not the form is waiting for async validation to
 * complete.
 * @return {boolean}
 */
BaseForm.prototype.isPending = function() {
  return Object.keys(this._pendingAsyncValidation).length > 0
}
...
/**
 * Determines whether or not the form is waiting for async validation of its
...
/**
 * Configures cross-field cleaning to run after its dependent fields have been
 * cleaned, or after all fields have been cleaned if dependencies have not been
 * configured.
...
BaseForm.prototype._preCleanFields = function(fieldNames) {
  ...
  if (typeof this.clean.fields != 'undefined') {
    ...
    // Ignore any invalid fieldnames given
    for (i = 0, l = fieldNames.length; i < l; i++) {
...
BaseForm.prototype._cleanField = function(name) {
  ...
  try {
    ...
    // Check for clean<Name>([cb]) or clean_<name>([cb])
    var customClean =
      (this['clean' + name.charAt(0).toUpperCase() + name.substr(1)] ||
       this['clean_' + name])
...
/**
 * Callback for completion of field cleaning.
 * @param {string} fieldName
 * @param {truthy} error truthy if an error ocurred while cleaning the field.
 */
BaseForm.prototype._fieldCleaned = function(fieldName, error) {
  if (error) {
    if ("production" !== process.env.NODE_ENV) {
  ...
  // If whole-form cleaning is waiting on other fields to be cleaned, run it if
  // this was is last field it's waiting on.
  if (this._runCleanAfter[fieldName]) {
    delete this._runCleanAfter[fieldName]
    if (Object.keys(this._runCleanAfter).length === 0) {
      this._cleanForm()
    }
  }
}
 ...
 * @return {boolean} true if cleaning is running asychronously
 */
BaseForm.prototype._runCustomClean = function(fieldName, customClean) {
  ...
  var callback = function(err, field, error) {
...
BaseForm.prototype._cleanForm = function() {
  try {
    if (this.clean !== noop) {
      this._runCustomClean(NON_FIELD_ERRORS, this.clean)
    }
  }
  catch (e) {
    if (!(e instanceof ValidationError)) {
...
BaseForm.prototype._cancelPendingOperations = function() {
  Object.keys(this._pendingFieldValidation).forEach(function(field) {
    object.pop(this._pendingFieldValidation, field).cancel()
  }.bind(this))
...
BaseForm.prototype._handleFieldEvent = function(validation, e) {
  ...
  // Bail out early if the event is only being handled to updae the field's data
  ...
  if (validation.event == 'onBlur') {
    ...
    if (typeof this._pendingFieldValidation[fieldName] != 'undefined') {
      this._pendingFieldValidation[fieldName].trigger()
      return
    }
  ...
  if (!validate && typeof this._pendingFieldValidation[fieldName] != 'undefined') {
    object.pop(this._pendingFieldValidation, fieldName).cancel()
  }
...
BaseForm.prototype._delayedFieldValidation = function(fieldName, delay) {
  if (typeof this._pendingFieldValidation[fieldName] == 'undefined') {
    this._pendingFieldValidation[fieldName] = util.debounce(function() {
      delete this._pendingFieldValidation[fieldName]
      this._immediateFieldValidation(fieldName)
    }.bind(this), delay)
  }
  this._pendingFieldValidation[fieldName]()
}
...
BaseForm.prototype._htmlOutput = function(normalRow, errorRow) {
  ...
  // Put a cross-field pending indnicator in its own row
